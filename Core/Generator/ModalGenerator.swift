...
public struct ModelGenerator {
     ...
     */
    func generateModelForJSON(_ object: JSON, _ defaultClassName: String, _ isTopLevelObject: Bool, _ rescursiveName: String = "") -> [ModelFile] {
        let className = NameGenerator.fixClassName(defaultClassName, configuration.prefix, isTopLevelObject, rescursiveName)
        ...
        if let rootObject = object.array, let firstObject = rootObject.first {
            ...
            if subClassType == .object {
                return generateModelForJSON(JSONHelper.reduce(rootObject), defaultClassName, isTopLevelObject, rescursiveName)
            }
            return []
        }
